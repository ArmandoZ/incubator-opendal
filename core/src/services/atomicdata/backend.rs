...
const DOCUMENT_CLASS: &str = "https://atomicdata.dev/classes/Document";
...
impl Adapter {
    ...
    fn sanitize_path(&self, path: &str) -> String {
        ...
        let binding = sanitize_filename::sanitize(path.to_owned());
...
impl Adapter {
    ...
    pub fn atomic_post_object_request(
        ...
        // Post
        let req = req.body(AsyncBody::Bytes(body)).unwrap();
...
impl kv::Adapter for Adapter {
    ...
    async fn set(&self, path: &str, value: &[u8]) -> Result<()> {
        ...
        // Real set
        let req = self.atomic_post_object_request(&path, value).unwrap();
